...
function metaFor(path) {
  ...
  if (path === "/guide") {
    return {
      title: `戦術ライブラリとは | ハンドボール戦術動画を整理して見返せる無料サービス`,
      description:
        "ハンドボールの戦術動画を局面・システム・チーム・代表・選手で整理し、見たいときにすぐ引き出せる無料サービスです。8分以内のクリップを自動再生するショート機能や、戦術の解説記事も掲載しています。",
    };
  }
  if (path === "/sponsor") {
    return {
      title: `スポンサー募集 | ${SITE_NAME}`,
      description:
        "ハンドボール戦術動画サイト「戦術ライブラリ」のスポンサーバナー募集のご案内。指導者・選手・保護者に届く掲載枠をご用意しています。",
    };
  }
  if (path === "/about") {
    return {
